...
export default defineEventHandler(async (event) => {
  try {
    ...
    const { sj_chuli_ren, sj_chuli_jieguo, sj_beizhu } = sjBody
    ...
    if (sj_chuli_ren !== undefined) {
      updateData.sj_chuli_ren = sj_chuli_ren
    }

    if (sj_chuli_jieguo !== undefined) {
      updateData.sj_chuli_jieguo = sj_chuli_jieguo
    }
    ...
    console.error('sj-jinji更新处理状态错误:', sjError)
