...
export default defineEventHandler(async (event) => {
  try {
    ...
    if (sj_zhuangtai === undefined || (sj_zhuangtai !== 0 && sj_zhuangtai !== 1)) {
      setResponseStatus(event, 400)
      return {
        sj_code: 400,
        sj_message: '状态值必须是0或1',
        sj_error: 'SJ_USER_001'
      }
    }
    ...
    return {
      ...
      sj_data: {
        sj_id: sj_siji.sj_id,
        sj_zhuangtai: sj_siji.sj_zhuangtai
      }
    }
  } catch (sj_error: any) {
    console.error('sj-user siji status error:', sj_error)
