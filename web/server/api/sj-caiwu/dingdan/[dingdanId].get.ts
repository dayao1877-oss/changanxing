...
export default defineEventHandler(async (event) => {
  try {
    ...
    if (!dingdanId || isNaN(Number(dingdanId))) {
      ...
      return { sj_code:400, sj_message:'订单ID无效', sj_error:'参数错误' }
    }
    const sjCaiwuList = await prisma.sjCaiwuJilu.findMany({ where: { sj_dingdan_id: Number(dingdanId) }, orderBy: { sj_shijian: 'desc' } })
    const sjResult = sjCaiwuList.map(item => sjConvertBigInts(item))
    return { sj_code: 200, sj_message: '查询成功', sj_data: sjResult }
  } catch (sjError) {
    console.error('sj-caiwu 查询订单错误:', sjError)
