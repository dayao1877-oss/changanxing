...
export default defineEventHandler(async (event) => {
  try {
    ...
    if (chuliZhuangtai !== undefined && chuliZhuangtai !== null) {
      ...
      if (!isNaN(zhuangtai) && [0, 1, 2].includes(zhuangtai)) {
        whereCondition.sj_chuli_zhuangtai = zhuangtai
      }
    }

    const sjYichangList = await prisma.sjYichangShangbao.findMany({
      where: whereCondition,
      orderBy: { sj_shangbao_shijian: 'desc' }
    })

    const sjResult = sjYichangList.map(item => sjConvertBigInts(item))
    ...
    console.error('sj-yichang查询司机异常错误:', sjError)
