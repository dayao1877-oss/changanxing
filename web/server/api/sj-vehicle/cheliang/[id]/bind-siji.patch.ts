...
export default defineEventHandler(async (event) => {
  try {
    ...
    if (sj_siji_id !== undefined) {
      const sj_exist_siji = await prisma.sjSiji.findUnique({ where: { sj_id: sj_siji_id } })
      if (!sj_exist_siji) {
        setResponseStatus(event, 400)
        return { sj_code: 400, sj_message: '司机不存在', sj_error: 'SJ-VEHICLE-004' }
      }
    }
    ...
    return {
      ...
      sj_data: {
        ...
        sj_siji_id: sj_cheliang.sj_siji_id
      }
    }
  } catch (sj_error: any) {
